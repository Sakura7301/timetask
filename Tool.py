...
class ExcelTool(object):
    ...
    def readExcel(self, file_name=__file_name, sheet_name=__sheet_name):
        ...
        if os.path.exists(workbook_file_path):
            ...
            if data is None or len(data) == 0:
                logger.info("[timeTask] 数据库timeTask任务列表数据为空")

            return data
        else:
            logger.info("timeTask文件不存在, 读取数据为空")
            self.create_excel()
            return []
    ...
    def moveTasksToHistoryExcel(self, tasks, file_name=__file_name, sheet_name=__sheet_name, history_sheet_name=__history_sheet_name):
        ...
        if os.path.exists(workbook_file_path):
            ...
            logger.info(f"将任务Sheet({sheet_name})中的 过期任务 迁移指 -> 历史Sheet({history_sheet_name}) 完毕~ \n 迁移的任务ID为：{hisIds}")

            #返回最新数据
            return self.readExcel()
        else:
            logger.info("timeTask文件不存在, 数据为空")
            self.create_excel()
            return []
    ...
    def addItemToExcel(self, item, file_name=__file_name, sheet_name=__sheet_name):
        ...
        if os.path.exists(workbook_file_path):
            ...
            logger.info("timeTask文件不存在, 添加数据失败")
            self.create_excel()
            return []
    ...
    def write_columnValue_withTaskId_toExcel(self, taskId, column: int, columnValue: str,  file_name=__file_name, sheet_name=__sheet_name):
        ...
        if len(data) > 0:
            ...
            logger.info("timeTask文件无数据, 消费数据失败")
            return False, None
...
class TimeTaskModel:
    ...
    def __init__(self, item, msg : ChatMessage, isNeedFormat: bool, isNeedCalculateCron = False):
        ...
        #容错
        temp_str = ""
        self.fromUser = temp_str if self.fromUser is None else self.fromUser
        self.fromUser_id = temp_str if self.fromUser_id is None else self.fromUser_id
        self.toUser = temp_str if self.toUser is None else self.toUser
        self.toUser_id = temp_str if self.toUser_id is None else self.toUser_id
        self.other_user_nickname = temp_str if self.other_user_nickname is None else self.other_user_nickname
        self.other_user_id = temp_str if self.other_user_id is None else self.other_user_id
        self.isGroup = False if self.isGroup is None else self.isGroup
        self.originMsg = temp_str if self.originMsg is None else self.originMsg
        ...
        if isNeedFormat:
            ...
            new_tuple = (self.timeStr, self.circleTimeStr, self.eventStr, self.fromUser, self.toUser, self.other_user_id, "1" if self.isGroup else "0")
            temp_content='_'.join(new_tuple)
            short_id = self.get_short_id(temp_content)
            logger.info(f'消息体：{temp_content}， 唯一ID：{short_id}')
            self.taskId = short_id

            #周期、time
            #cron表达式
            if self.isCron_time():
                logger.info("cron 表达式")

            else:
                #正常的周期、时间
                g_circle = self.get_cycle_day(self.circleTimeStr)
    ...
    def get_todayCron_times(self):
        ...
        if self.isValid_Cron_time():
            ...
            while next_time.date() == current_time.date():
                #记录时间（时：分）
                next_time_hour_minute = next_time.strftime('%H:%M')
                self.cron_today_times.append(next_time_hour_minute)
                next_time = cron.get_next(datetime)
    ...
    def is_today(self):
        ...
        if self.is_valid_date(item_circle):
            ...
            if item_circle == current_time.format('YYYY-MM-DD'):
                #今天要出发的任务
                #logger.info(f"[定时任务]类型: 录入日期, 日期信息：{item_circle}")
                return True
            else:
                #其他时间待出发
                #logger.info(f"[定时任务]类型: 录入日期, 非今天任务, 日期信息：{item_circle}")
                return False

        elif "每天" in item_circle:
            #今天要出发的任务
            #logger.info(f"[定时任务]类型：每天")
            return True

        elif "每周" in item_circle or "每星期" in item_circle:
            if self.is_today_weekday(item_circle):
                #logger.info(f"[定时任务]类型: 每周, 日期信息：{item_circle}")
                return True
            else:
                #logger.info(f"[定时任务]类型: 每周, 非今天任务, 日期信息为：{item_circle}")
                return False
                ...
                if is_weekday:
                    #logger.info(f"[定时任务]类型: 工作日")
                    return True
                else:
                    #logger.info(f"[定时任务]类型: 工作日, 非今天任务，日期信息为：{item_circle}")
                    return False
    ...
    #获取周期
    def get_cycle_day(self, circleStr):
        ...
        #每周X
        elif circleStr in ["每周一", "每周二", "每周三", "每周四", "每周五", "每周六", "每周日", "每周天", "每星期一", "每星期二", "每星期三", "每星期四", "每星期五", "每星期六", "每星期日", "每星期天"]:
            #每天、每周X等
            g_circle = circleStr
    ...
    def get_gropID_withGroupTitle(self, groupTitle, channel_name):
        ...
        if channel_name == "wx":
            ...
            try:
                ...
                logger.info(f"[{channel_name}通道] 通过 群Title 获取群ID发生错误，错误信息为：{e}")
                return tempRoomId
            ...
            logger.info(f"[{channel_name}通道] 通过 群Title 获取群ID 不支持的channel，channel为：{channel_name}")
            return ""
