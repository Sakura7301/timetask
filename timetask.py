...
class timetask(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        if self.channel is None:
            ...
            logger.debug(f"本次的channel为：{self.channel}")
    ...
    def get_timeInfo(self, content):
        ...
        if content.startswith("cron[") or content.startswith("Cron[") :
            ...
            if cron_end_index != -1:
                ...
                logger.info("cron表达式 格式异常！")

        else:
            #分割
            wordsArray = content.split(" ")
            if len(wordsArray) <= 2:
                logger.info("指令格式异常，请核查")
            else:
                #指令解析
                #周期
    ...
    def runTimeTask(self, model: TimeTaskModel):
        ...
        if model.isPerson_makeGrop():
            ...
            isGroup = True
            if len(groupId) <= 0:
                logger.error(f"通过群标题【{groupTitle}】,未查到对应的群ID, 跳过本次消息")
                return

        logger.info("触发了定时任务：{} , 任务详情：{}".format(model.taskId, eventStr))
        ...
        if is_open_route_everyReply:
            try:
                ...
                logger.info(f"开启了所有回复均路由，但是消息路由插件异常！后续会继续查询是否开启拓展功能。错误信息：{e}")
        ...
        if is_open_extension_function and route_replyType is None:
            ...
            for item in funcArray:
                ...
                if event_content.startswith(key_word):
                    event_content = event_content[:event_content.find(key_word)] + func_command_prefix + key_word + event_content[event_content.find(key_word) + len(key_word):]
                    event_content = event_content.strip()
                    isFindExFuc = True
                    break
